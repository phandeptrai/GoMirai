...
public class MapboxService {
    ...
    @org.springframework.cache.annotation.Cacheable(
        value = "mapboxDirections", 
        key = "#request.origin.latitude + ',' + #request.origin.longitude + '|' + #request.destination.latitude + ',' + #request.destination.longitude + '|' + #request.profile",
        unless = "#result == null"
    )
    public RouteResponse getDirections(RouteRequest request) {
        if (request == null || request.getOrigin() == null || request.getDestination() == null) {
            throw new BusinessException("Origin and destination are required");
        }

        try {
            String profile = request.getProfile() != null ? request.getProfile() : "driving";
            String coordinates = formatCoordinates(request.getOrigin(), request.getDestination());
            ...
            log.debug("Calling Mapbox Directions API: {}", url.replace(accessToken != null ? accessToken : "", "***"));
            ...
            throw new BusinessException("Failed to get directions from Mapbox API: " + response.getStatusCode());
            ...
            throw new BusinessException("Failed to calculate route: " + e.getMessage());
        }
    }
    ...
    private String formatCoordinates(GeoPoint origin, GeoPoint destination) {
        return String.format(java.util.Locale.US, "%f,%f;%f,%f",
    ...
    private RouteResponse parseDirectionsResponse(String json) throws Exception {
        ...
        if (routes.isMissingNode() || !routes.isArray() || routes.isEmpty()) {
            throw new BusinessException("No route found between these points.");
        }

        JsonNode route = routes.get(0);
        JsonNode legs = route.path("legs");
        
        if (legs.isMissingNode() || !legs.isArray() || legs.isEmpty()) {
            throw new BusinessException("Invalid route data received from map service.");
        }
